...
def parse_args():
    p = argparse.ArgumentParser(
        description="Validate SKU detection on bin images (STRICT)"
    )
    ...
    # default behaviour: if nothing specified, use 00015 (quick test)
...
def process_single_bin(
    ...
    - runs DINO + CLIP
    - saves overlay image
    - returns summary rows (list of dicts)
    """
    image_path = BIN_DIR / f"{image_id}.jpg"
    meta_path = META_DIR / f"{image_id}.json"
    out_image_path = OUT_DIR / f"{image_id}_detected_v1_strict.jpg"
    ...
    # Count visible clusters (not guessing full quantities)
    ...
    for item in items:
        ...
        # Aggressively shorten for DINO + CLIP
        ...
        boxes_norm, logits, _ = predict(
            ...
            device=device,  # keep DINO on same device
        )
        ...
        # Prepare CLIP text embedding (once per SKU) using shortened name
        with torch.no_grad():
    ...
    # ---------- SUMMARY (STRICT) ----------
    print("\n📊 SKU VERIFICATION SUMMARY (STRICT):\n")
    rows = []
    for item in items:
        asin = item["asin"]
