...
# ---------------- CONFIG (V4: CLIP 0.18 + DINO-LOOSE) ----------------
...
# GroundingDINO thresholds (same as v3: looser than default)
BOX_THRESHOLD = 0.15
TEXT_THRESHOLD = 0.15

# CLIP validation (v4: more relaxed than v2/v3)
CLIP_THRESHOLD = 0.18
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Validate SKU detection on bin images (V4: CLIP 0.18 + DINO-loose)"
    )
...
def process_single_bin(
    ...
    out_image_path = OUT_DIR / f"{image_id}_detected_v4_clip018.jpg"
    ...
    print("\n📊 SKU VERIFICATION SUMMARY (V4 CLIP 0.18 + DINO-loose):\n")
    rows = []
    for item in items:
        asin = item["asin"]
...
def main():
    ...
    summary_path = OUT_DIR / "summary_v4_clip018.csv"
