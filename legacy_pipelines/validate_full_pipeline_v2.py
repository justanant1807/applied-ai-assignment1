...
def parse_args():
    p = argparse.ArgumentParser(
        description="Validate SKU detection on bin images (RELAXED)"
    )
...
def process_single_bin(
    ...
    out_image_path = OUT_DIR / f"{image_id}_detected_v2_relaxed.jpg"
    ...
    print("\n📊 SKU VERIFICATION SUMMARY (RELAXED):\n")
    rows = []
    for item in items:
        asin = item["asin"]
...
def main():
    ...
    summary_path = OUT_DIR / "summary_v2_relaxed.csv"
